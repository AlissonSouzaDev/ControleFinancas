      ...
      <div className="flex-1 overflow-y-auto divide-y divide-gray-50 dark:divide-slate-700">
        ...
        {registros.map(r => (
          <div
            ...
            className={`flex items-center border border-gray-200 dark:border-slate-700 gap-4 px-5 py-3 cursor-pointer transition-colors ${selecionado === r.id ? 'bg-blue-100 dark:bg-slate-700' : 'hover:bg-green-50 dark:hover:bg-slate-700/60'}`}
          >
            <div className="bg-[#E8F5E9] dark:bg-green-900/30 p-2 rounded-full flex-shrink-0">
              <ArrowUpCircle size={18} className="text-[#43A047]" strokeWidth={2} />
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-[#1C2B3A] dark:text-slate-100 truncate">{r.descricao}</p>
              <p className="text-xs text-gray-500 dark:text-slate-300">{r.data.split('-').reverse().join('/')}</p>
            </div>
            <p className="text-sm font-semibold text-[#43A047] flex-shrink-0">{formatarMoeda(r.valor)}</p>
          </div>
        ))}
      </div>
    </div>
