    ...
    <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-300 dark:border-slate-700 shadow-sm flex flex-col transition-colors overflow-hidden">
      <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 dark:border-slate-700">
        <div className="flex items-center gap-3">
          <span className="w-[4.5px] h-5 bg-green-500 rounded-full" />
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 dark:bg-slate-700/50 text-xs text-gray-500 dark:text-slate-100 uppercase tracking-wide">
                ...
                <th className="px-4 py-2 text-right font-medium whitespace-nowrap">Valor Recebido</th>
              ...
              {registros.map(r => (
                <Fragment key={r.id}>
                  <tr
                    ...
                    className={`cursor-pointer transition-colors ${selecionado === r.id ? 'bg-blue-100 dark:bg-slate-700' : 'hover:bg-green-50 dark:hover:bg-slate-700/60'}`}
                  >
                    ...
                    <td className="px-4 py-3">
                      <StatusPagamento status={r.status_pagamento} tipo="a_receber" />
                    </td>
                    <td className="px-4 py-3 text-right text-gray-500 dark:text-slate-100">{formatarMoeda(r.valor_realizado)}</td>
                    <td className={`px-4 py-3 text-right font-semibold ${r.prioridade ? 'text-[#43A047]' : 'text-[#1C2B3A] dark:text-slate-100'}`}>
                  ...
                  {selecionado === r.id && (
                    <tr>
                      <td colSpan={5} className="px-4 py-3 bg-blue-50 dark:bg-slate-700/40 border-t border-blue-100 dark:border-slate-600">
                        <p className="text-xs font-semibold text-gray-500 dark:text-slate-400 mb-1">Observação</p>
                        <textarea
                          readOnly
                          value={r.observacao ?? ''}
                          placeholder="Nenhuma observação registrada."
                          rows={2}
                          className="w-full text-sm text-gray-700 dark:text-slate-300 bg-transparent resize-none outline-none placeholder:text-gray-400 dark:placeholder:text-slate-500"
                        />
                      </td>
                    </tr>
                  )}
